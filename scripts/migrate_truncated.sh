...
function migrateToCDS() {
    ...
    local siteAdminUserName=${2}
    local siteAdminPassword=${3}
    local upgradeCategory=${4}
    local uploadTaskUuid=${5}
    local cdiName=${6}
    local envUrn=${7}
    local orgId=${8}
    local refreshToken=${9}

    local trial=$(retrieveUnclaimedTrial $orgId $refreshToken)
    local token=$(getJwtToken $refreshToken)
    #local databaseSchemaVersion=$(getSupportedDatabaseSchemaVersions $primaryCell $cellUserName $cellUserPassword)
    local databaseSchemaVersion='[ "2.1.9", "6.0.116", "7.0.0", [ "1.0.44", "45.2.0", [ "19.9.0", "11.0.0", "8.0.0" ], [ "6.0.0", "6.0.0"], [ "14.0.0", "4.0.0", "6.0.0", "9.0.0", "5.0.0" ], [ "7.0.0", "3.0.0", "4.0.0", "12.0.0", "2.0.0", "2.0.0", "7.0.0", "3.0.0", "6.0.0", "5.0.0", "15.0.0", "5.0.0", "21.0.0", "84.0.0" ], [ "46.0.0", "100.0.0", "88.0.0" ], [ "64.0.0", "66.0.0", "149.0.0", "69.0.0", "23.0.0", [ "36.0.0", "2.0.0" ] ] ], "364.0.0" ]'
    local data=''
    if [[ -z "$trial" ]]; then
        data="{
            \"name\": \"${cdiName}\",
